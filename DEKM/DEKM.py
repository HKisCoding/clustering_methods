...
def model_conv(load_weights=True):
    ...
    h = x
    x = layers.Dense(
        filters[2] * (input_shape[0] // 8) * (input_shape[0] // 8), activation="relu"
    )(x)
...
def train(x, y):
    ...
    for ite in range(int(140 * 100)):
        if ite % update_interval == 0:
            ...
            # log
            log_str = (
                f"iter {ite // update_interval}; acc, nmi, ri = {acc, nmi, loss}; loss:"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="select dataset:MNIST,COIL20,FRGC,USPS"
    )
